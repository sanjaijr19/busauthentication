...
from account.form1 import DriverForm,CustomerForm
from django.contrib.auth import authenticate, login
# Create your views here.
from django.shortcuts import render,redirect,HttpResponse
from account.models import Driver,Customer,User
...
def empDetails(request):
    ...
    return render(request, "index1.html", context =emp_dict)
...
def driverDetails(request):
    emp_data=Driver.objects.all()
    emp_dict = {"emp_list":emp_data}
    return render(request, "driver.html", context =emp_dict)
#u
...
def update_view(request,id):
    ...
    if request.method == "POST":
        ...
        emp_data.save()
        return redirect("/home")
    return render(request, "update.html",{"emp_data":emp_data})
...
def consumerdisplay_view(request):
    ...
    return render(request,"consumerdisplay.html",{"info":info})
...
def consumer_view(request):
    ...
    return render(request,'consumeradd.html',{'form1':form1})
...
def updateconsumer_view(request, id):
        ...
        if request.method == ('POST'):
            ...
            consumer_data.save()
            return redirect('/consumerdisplay')
        return render(request, 'consumerupdate.html', {"consumer_data": consumer_data})

def customerview(request):
    info = Customer.objects.all()
    return render(request, "customerview.html", {"info": info})
